...
public class Collections {
     ...
     */
    public static <T> //二分查找的前提是保证Collection有序
    int binarySearch(List<? extends Comparable<? super T>> list, T key) {
        if (list instanceof RandomAccess || list.size()<BINARYSEARCH_THRESHOLD) //判断是否为RandomAccess的实例
            return Collections.indexedBinarySearch(list, key); //普通遍历
        else
     ...
     */
    @SuppressWarnings({"rawtypes", "unchecked"})
    public static void swap(List<?> list, int i, int j) { //交换元素的位置
        // instead of using a raw type here, it's possible to capture
        // the wildcard but it will require a call to a supplementary
        // private method
        final List l = list;
     ...
     * @since  1.4
     */
    public static int indexOfSubList(List<?> source, List<?> target) { //第一次出现的索引，找不到则-1
     ...
     * @serial include
     */
    static class UnmodifiableCollection<E> implements Collection<E>, Serializable { //装饰器模式
