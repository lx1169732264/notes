 ...
 * @since   1.2
 */
public class ThreadLocal<T> { //通过每个线程保存变量的副本,空间换时间     天生支持线程隔离,只有本线程能够获得它设置的值    适合以线程作为作用域,不同线程具有不同数据副本的场景
    /**
    ...
    static class ThreadLocalMap {
         ...
         */
        static class Entry extends WeakReference<ThreadLocal<?>> { //存储[ThreadLocal<?>>,变量]的二元组     相较于HashMap,没有链表结构,只是单纯的键值关系     通过弱引用提高ThreadLocalMap被回收的几率
        ...
        private void replaceStaleEntry(ThreadLocal<?> key, Object value,
            ...
            int slotToExpunge = staleSlot; //扫描的起始位置
         ...
         */
        private boolean cleanSomeSlots(int i, int n) { //清理key==null的数据
