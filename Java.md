...
CPU从内存取数据到寄存器，然后进行处理，但由于内存的处理速度远低于CPU，导致CPU在处理指令时往往花费很多时间在等待内存做准备工作，于是在寄存器和主内存间添加了**CPU缓存**，CPU缓存比较小，但访问速度比主内存快。
...
* 虚拟机栈
  * 存储==对象的引用,基本类型,指令地址==,分配的内存大小是确定的
  * 线程私有，生命周期与线程相同,==方法调用时进栈,执行结束出栈==
  * 每个方法执行都会创建栈帧，存放局部变量表，操作栈，动态链接，方法出口等。
  * 效率高.由操作系统自动分配,在硬件上对栈提供支持：有专门的寄存器存放栈的地址，压栈出栈有专门指令
  * 按先后定义的顺序依次压栈，**相邻变量的地址之间不会存在其它变量**。栈的内存地址生长方向与堆相反，由高到底，**后定义的变量地址低于先定义的变量**
  * 两种异常StackOverFlowError和 OutOfMemoneyError
    * StackOverFlowError 线程请求栈深度大于虚拟机所允许的深度
    * OutOfMemoneyError 无法申请到足够的内存空间
...
* ==gc的主要区域==
  * Eden 放新创建对象,==分配内存时需要加锁==
    * 线程在Eden上被分配独享的空间TLAB（Thread Local Allocation Buffer）
    * ==在TLAB分配内存不需要加锁==，因此JVM在给线程中的对象分配内存时会尽量在TLAB上分配
    * 如果对象过大或TLAB用完，则仍然在堆上进行分配。如果Eden区内存也用完了，则进行Minor GC
  * From survivor / To survivor 保存gc后幸存对象，默认占比 8:1:1
    * Minor GC后，from和to互换,并把一些仍然存活的对象复制进to区，清除内存,再把to区一些存活得足够旧的对象移至年老代
  * 年老代  存活时间较久，较大的对象
...
* 只有1个,被线程共享
* 存储==已被虚拟机加载的class文件信息,常量，static变量==等**唯一的元素**
* GC主要针对常量池回收，类型卸载
...
* **线程私有**
* 当前线程所执行的行号指示器。通过改变计数器的值来确定下一条指令，比如循环，分支，跳转，异常处理，线程恢复

* 多线程是通过线程轮流切换并分配处理器执行时间实现。为了线程切换能恢复到正确的位置，每条线程都需要一个独立的程序计数器
