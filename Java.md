...
CPU从内存取数据到寄存器，然后进行处理，但由于内存的处理速度远低于CPU，导致CPU在处理指令时往往花费很多时间在等待内存做准备工作，于是在寄存器和主内存间添加了**CPU缓存**，CPU缓存小，但访问速度比主内存快
...
* 虚拟机栈
  * 存储==对象的引用,基本类型,指令地址,局部变量表，动态链接，方法出口==,分配的内存大小是确定的
