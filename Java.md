...
* 虚拟机是相对于物理机的概念，都有代码执行能力

* 物理机执行引擎直接建立在处理器、硬件指令集、操作系统层面

* 虚拟机的执行引擎是由自己实现，用于执行虚拟机字节码指令集

* 执行的时候可能解释（通过解释器执行）或编译（通过执行编译器产生本地代码执行），Jvm的执行引擎输入的是字节码文件，处理过程是字节码解析的等效过程，输出执行结果
...
* ==分配的内存大小是确定的==
  ...
  * ==方法返回地址==
    ...
    * 方法执行时，只有两种方式退出这个方法：正常/异常完成
      * 正常完成出口：执行引擎遇到一个方法返回的字节码指令，这时候执行引擎读取栈帧中的方法返回地址，将返回值传递给上层的方法调用者。
      * 异常完成出口：遇到异常并且没有在方法体内得到处理，也就是在本地异常表内没有搜索到匹配的异常处理器，就会导致方法退出。这时候执行引擎不会读取方法返回地址而直接停止执行，上层调用者不会得到任何返回值。
  
    * ==方法退出:把当前栈帧出栈。恢复上层方法的局部变量表和操作数栈，把返回值压入调用者栈帧的操作数栈中，调整PC计数器的值以执行下一条指令==
...
* 只有1个,被所有线程共享
* 存储==对象==和与之对应的class信息(操作指令)
* ==堆是gc的主要区域==
  * 新生代   分三个区,默认占比 8:1:1,方便采用**复制-清除策略**
  * 区分空闲/使用区,将存活的对象从使用区复制进空闲区，**避免碎片问题**。虽然复制后使用区没有碎片，但下一次GC，Eden区和使用区里都存在需要回收的对象,从而导致碎片
    ...
    * Eden主要存放新创建对象,==分配内存时需要加锁==
      ...
      * Eden满时
        ...
        * < 则判断参数HandlePromotionFailure(是否允许担保失败,默认允许)，有担保则**Minor GC**
          ...
          * 长期存活进入老年代，避免Eden和Survivor重复复制,Survivor空间不足  -XX:MaxTenuringThreshold= 默认15
          * **动态对象年龄判断**,年龄相同的对象>Survivor/2，**超出部分放入老年代,保证老年代空间充足**
      * GC后,使用/空闲区互换,**年龄+1**
  * 年老代  存活时间较久，较大的对象
    * 标记整理算法:容量满会触发Major GC（full GC），回收年老代和年轻代中不再被使用的对象资源
...
* ==永久代==
  * ==GC主要进行常量池回收，类型卸载==
* 具有一定的动态性，可以存放编译期生成的常量,如string的intern()
* 常量池,包含==基本类型和对象型的常量值==
...
* 方法表
  ...
  * 为了优化对象调用方法的速度，方法区的类型信息会增加指针，指向方法表，方法表指向具体方法,这些方法中包括从父类继承的所有方法以及自身重写（override）的方法
