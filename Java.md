...
将.java文件通过**javac编译**为.class文件 ( java的跨平台性 )
...
[类加载器加载字节码文件](#加载), 并在**堆内存**中生成**Class类对象**, 然后通过解释器逐行解释执行
...
| ---------- | ------------------------------------- | ---------------------------------- |
| 发生范围   | 同一个类                              | 子类                               |
|            | 方法名相同,参数不同, 返回类型可以不同 | 方法名, 参数, 返回类型相同         |
| 访问修饰符 | 无要求                                | 访问修饰符>被重写的方法            |
| 抛出异常   | 无要求                                | 不能抛出新异常 或 抛出更宽泛的异常 |
...
* 相同
  * **都不能实例化**
  * 都可以声明abstract或default的方法
  * 都可以声明成员变量, 但接口的成员变量会被隐式地加上`public static final`, 而抽象类的成员变量没有任何限制

* 不同
  * **接口不能定义构造方法,抽象类中可以有构造方法，但不能用来实例化**
  * ==单继承, 多接口==
  * 抽象类可以继承抽象类, 也可以实现接口; **接口不能继承抽象类, 但可以继承或实现实现接口**
  * 抽象类允许子类不重写父类方法; 接口的方法必须实现

| **关系**               | 是否允许 | 示例                                |
| :--------------------- | :------- | :---------------------------------- |
| 接口 → 继承 → 抽象类   | ❌ 禁止   | `interface A extends AbstractClass` |
| **接口 → 继承 → 接口** | ✅ 允许   | `interface A extends B, C`          |
| 抽象类 → 实现 → 接口   | ✅ 允许   | `abstract class A implements B`     |
| 抽象类 → 继承 → 具体类 | ✅ 允许   | `abstract class A extends B`        |
...
接口定义“做什么”，实现类负责“怎么做”,是一种**自上向下**的设计。**has-a **

抽象类是将子类的共有特性进行抽象,是**自下向上**的设计  **is-a**
...
**机器码**	硬件平台可以直接读取执行的指令. 同一指令在不同平台, 会对应不同的机器码

**指令集**	硬件平台支持的指令集合. 例如x86平台的x86指令集, ARM平台的ARM指令集

**汇编**		由于指令的可读性差, 又发明了**汇编语言**来代替指令. 汇编语言**经过汇编**后, 会转化成该平台上的机器码

**字节码**	字节码是一种中间状态的二进制代码. **javac编译器**将**源码**编译为字节码, **解释器**将字节码转义为机器码. 字节码为源码提供了**跨平台性**



## 编译与解释并存

编译型语言:	编译器针对特定的操作系统, 将源代码一次性翻译成可被该平台执行的机器码
...
![](image.assets/image-20240517153231342.png)

橙色部分由[javac编译器](#类加载)完成, 将源码编译为字节码. JVM可以根据是否为`热点代码`, 选择字节码的执行方式是解释还是编译

绿色部分由解释器完成, 将字节码解释为机器码
...
**parallelStream()：**并行流, 需要配合线程安全的容器使用. 底层使用的是ForkJoinPool
...
**在考虑HashMap时间复杂度时, 还需要考虑哈希冲突, 链表与红黑树互相转换的情况**
...
| 1.5倍扩容, 空间占用率低         | 链表天生不需要扩容, 100&的空间占用率 | 2倍扩容                         |

对于单个元素来说, LinkedList需要存储前后元素的指针, 占用的内存比ArrayList大

对于整个集合来说, ArrayList在扩容后, 数组中存在大量没利用到的空间. 而LinkedList是不需要扩容的, 不会存在浪费掉的空间
...
使用 **单一锁（对象级锁）**，所有操作串行化get/put等加了synchronized**锁住整个table**, 线程安全, 性能低
...
1. 首先由一个单线程**创建新数组`nextTable`**，新容量一般为原table容量的两倍
2. 然后根据`stride`分片阈值(默认16), 将**原table划分为若干个区间,** 为**每个区间分配一个线程**去操作迁移, 并通过`transferIndex` 记录当前迁移进度
3. 在开始迁移时, 会先用**synchronized锁住原table的头节点**(同一个桶只能由一个线程操作迁移)
4. 每个线程想增/删元素时，如果访问的桶是ForwardingNode节点，表明正在扩容，协助扩容
5. 扩容时将原table的所有桶倒序分配，每个线程每次最小分配16个桶，防止资源竞争。单个桶内元素的迁移是加锁的，但桶范围处理分配可以多线程，在没有迁移完成所有桶之前每个线程需要重复获取迁移桶范围，直至所有桶迁移完成
6. 一个旧桶内的数据迁移完成但不是所有桶都迁移完成时，查询数据委托给ForwardingNode结点查询nextTable完成
7. 迁移过程中sizeCtl用于记录参与扩容线程的数量，全部迁移完成后sizeCtl更新为新table容量的0.75倍
...
只有内核空间才能进行系统态级别的资源操作, 应用程序通常运行在用户空间, 如果应用程序需要执行IO操作, 需要依赖于**系统调用**来间接访问内核空间. 也就是说应用程序只是调用者, 具体的IO操作完全由内核来完成



当应用程序发起 I/O 调用后，会经历两个步骤. **步骤1阻塞即为同步IO, 除了异步IO外, 其他四种IO模型都是同步IO**
...
应用程序系统调用`read`后，会一直**阻塞**，直到数据从内核缓冲区复制到应用进程缓冲区中
...
* 应用进程循环进行系统调用`read`
  * 如果内核缓冲区没有数据则返回错误码, 应用进程继续执行
  * 如果有数据, 则应用进程阻塞, 直到内核将数据从内核缓冲区拷贝到用户空间

相较于BIO, NIO的CPU需要重复判断内核的数据是否准备完毕, CPU利用率↓



<img src="image.assets/bb174e22dbe04bb79fe3fc126aed0c61~tplv-k3u1fbpfcp-watermark.image" style="zoom: 50%;" />
...
> 多路	监视多个描述符，一旦某个描述符读就/写就绪，能够通知程序进行相应的读写操作
>
> 复用	复用Selector一个线程监听多个线程的IO请求

1. 应用进程发起`select`系统调用，并阻塞. 该IO请求被注册到**Selector多路复用器**上, Selector负责判断内核数据是否准备就绪
2. 内核把数据准备完毕后，select调用就会返回, 通知应用进程进行读取
3. 应用进程发起`read`调用

> select, poll，epoll都是支持多路复用的系统调用, 并且都是**同步**IO
>
> - **select** ：O(n)  监听的最大连接数存在限制. 系统都支持select,但不一定支持另外两种系统调用
> - poll: O(n)  支持无限的连接数, 但需要对这些连接进行循环, 当数量过多时存在性能问题
> - **epoll** ：O(1)  linux 2.6之后才支持，事件驱动不阻塞，mmap实现内核与用户空间的消息传递，数量很大

**通过Selector实现一个线程便可以管理n个IO请求, 相较于NIO也没有循环的系统调用，减少了对 CPU 资源的消耗**



<img src="image.assets/88ff862764024c3b8567367df11df6ab~tplv-k3u1fbpfcp-watermark.image" style="zoom:50%;" />



#### select  O(n)

通过设置或者检查 存放**文件描述符的数组** 来判断流是否可以读写。这个数组大小的上限为`FD_SETSIZE`,默认是1024个. 所以**select的最大连接数有限**

它只知道有I/O事件发生了，却不知道对应的是数组中的哪个文件描述符，所以需要**轮询数组**，找出读/写准备就绪的文件并进行操作. 在没有就绪文件的情况下, select线程将被挂起，直到有设备就绪或者主动超时, 被唤醒后又会再次轮询



NIO是由用户进程来判断是否存在准备就绪的fd, 而select则交给了内核来判断	



#### poll  O(n)

监听IO事件的逻辑与select一致. 唯一区别是用**链表存储文件描述符**, 所以**连接数量没有限制**, 但它依然需要轮询链表
...
* epoll_create创建一个epoll句柄, 通过mmap开辟一块共享空间, 允许被内核和用户进程共享. 空间中用**红黑树**存储了所有监听的文件描述符, 用**链表**存储准备就绪的文件描述符
* epoll_ctl  注册要监听的事件类型, 往红黑树中插入文件描述符
* epoll_wait  线程挂起并等待唤醒



epoll流程

1. 线程调用`epoll_wait`, 挂起并等待唤醒
2. 文件描述符读写就绪后, 调用**callback函数将fd存入到链表中**, 并唤醒`epoll_wait`线程
3. `epoll_wait`线程只需要遍历链表就能拿到所有就绪的文件描述符

epoll和select/poll一样需要轮询fd, 直到有文件准备就绪, 在此期间线程会被反复地挂起/唤醒. 不过epoll利用了回调函数, 只需要遍历准备就绪的fd链表, 而select/poll需要遍历所有fd
...
当 epoll_wait() 检测到描述符事件到达后，将此事件通知进程，进程**可以不立即处理**该事件，下次调用 epoll_wait() 会**再次通知**进程。同时支持 Blocking 和 No-Blocking

进程可能会收到大量自己不关心的文件描述符的读写就绪事件, 效率低

2. EPOLL**ET** 高速模式

通知描述符事件到达后, 进程必须**立即处理**事件，下次再调用 epoll_wait() 时**不再通知**

**减少了epoll事件重复触发次数**, 效率高。只支持 No-Blocking，以避免由于一个文件句柄的阻塞读/阻塞写操作把处理多个文件描述符的任务饿死
...
在监听的文件描述符较少情况下, 遍历它们并不耗时, 而epoll回调函数的通知机制会带来额外的开销, 此时select/poll > epoll

在文件描述符较多的情况下, select/poll的遍历比较耗时
...
应用进程进行`sigaction`系统调用 ，内核立即返回不阻塞应用进程。内核在**数据到达**时向应用进程发送 SIGIO 信号，通知应用进程调用 recvfrom 将数据从内核复制到应用进程中
...
字符输入流和字节输入流的组成非常相似，文件存储/网络传输的最小单元都是字节

**JVM为字节流转化字符流的编码提供了支持**, 字符流默认采用 `Unicode` 编码, 而`utf8` :英文占 1 字节，中文占 3 字节，`unicode`：任何字符都占 2 个字节，`gbk`：英文占 1 字节，中文占 2 字节

![](image.assets/a4bb9f2c63c28876868e7b2fcd53e597.png-wh_600x-s_8800480.png)
...
支持随意跳转到文件的任意位置进行读写, 并且支持指定读写模式

RandomAccessFile通常用于**分片上传**的文件合并
...
//openAndDelete:打开文件并且这个文件后是否会被删除
...
r: 读	w: 写	s: 保存文件内容以及元数据(文件大小/修改时间等)	d:只保存文件内容	
...
| 面向流,字节                                                  | 面向**块**,数据块                                            |
| 阻塞,连接:线程=1:1                                           | 非阻塞,==异步==                                              |
| 没处理完线程不能退出,**不能用线程池**,连接数较多将出现资源不足 | 基于Selector多路复用, 允许一个线程监视多个输入通道，**可用线程池** |
| 瓶颈在于线程不足,不能处理过多的连接                          |                                                              |
|                                                              | 缓冲区可以分片，只读/直接/间接缓冲区                         |
...
![img](image.assets/093f9e57-429c-413a-83ee-c689ba596cef.png)
...
序列化就是**对象转化为二进制字节流**的过程. 因为网络传输或数据存储都是以二进制字节流的形式进行的
...
**socket/thread这两种特殊对象无法序列化**，即使序列化了也无法对它们重新分配资源
...
1. ~~实现Serializable接口~~, 这种方式只支持java语言, 并且序列化后的字节比较大, 不利于网络传输

   需要手动指定一个serialVersionUID
...
==cpu资源的最小调度单位是线程,系统资源(比如内存)的最小分配单元是进程==
...
1. 将变量从主内存拷贝到工作内存
2. 对工作内存的变量进行读写,此时因为==线程间无法访问对方的工作内存，线程通信(传值)必须通过主内存完成==,导致多线程对共享变量修改时相互不可见
3. 将工作内存的变量回写到主内存
...
- **并发**：n个作业在同一**时间段**内执行 (看似同时执行, 实则**交替**执行)
- **并行**：n个作业在同一**时刻**执行 (同时执行)



**守护线程 daemon**

GC线程	JIT编译线程	JMX管理线程	ForkJoinPool.commonPool工作线程

**守护线程的生命周期与JVM一致**, JVM中只剩守护线程时将会强制退出, 不会等待守护线程的方法执行完毕, 所以守护线程里不要放任何关键操作(文件/数据库写入)
...
AbstractQueuedSynchronizer 抽象队列式同步器,**无锁**,除synchronized之外的锁都基于AQS. 由**state 变量 +同步队列+等待队列(结合Condition使用)**组成
...
**同步队列是双向链表**, 线程通过CAS改变volatile的state，失败则进入同步队列，等待唤醒

**等待队列是单向链表**, 条件condition不满足时进入等待队列, 满足条件后被signal/signalAll唤醒加入同步队列

==同步队列的线程会参与CPU时间片的竞争, 等待队列的线程不参与竞争, 彻底释放了CPU资源==
...
![](image.assets/_AQS_weikunkun_02.png)
...
| :------- | :--------------------- | :------------------------------------- | :------------------------- |
| 用途     | 等待一组操作完成       | 通过**许可证**对资源的访问进行**限流** | 让线程在**屏障**前互相等待 |
| 初始化值 | 固定数量（构造时指定） | 可**动态调整许可证数量**               | 固定数量（构造时指定）     |
| 计数方向 | 递减                   | 可增可减                               | /                          |
| 重用性   | **不可重用**           | 可重用                                 | 可重用                     |
| 主要方法 | await(), countDown()   | acquire(), release()                   | await(), reset()           |
...
| 比较点       | Thread                     | Runnable                           |
| :----------- | :------------------------- | :--------------------------------- |
| **继承关系** | 继承Thread类               | 实现Runnable接口                   |
| **灵活性**   | 不支持多继承，扩展性差     | 可以继承其他类，灵活性高           |
| **资源共享** | 每个Thread对象是独立的     | 多个线程可以共享同一个Runnable实例 |
| **内存消耗** | 每个Thread对象占用更多资源 | 更节省资源                         |



### sleep

==不释放锁==, 必须指定超时时间, 超时**自动唤醒**
...
本质是两个线程之间用wait/notifyAll实现等待机制

==不释放当前线程已持有的锁, 会释放目标线程的内置对象锁==



1. a线程中调用b.join(), 因为join被synchronized修饰, a线程获取b的**对象锁**
2. a判断b是否Alive
   1. b终止, a继续执行
   2. b未终止, a调用**`wait`释放对象锁**并进入等待. b执行结束后通过**JNI回调`notifyAll`唤醒a**
...
==不释放锁==, 提示调度器当前线程愿意让出CPU时间片, 但**不保证当前线程会被立即暂停**. 再次被分配到CPU时间片时即被唤醒
...
==线程隔离==	创建线程内私有的变量, 隔离了线程存取数据的行为, 使得线程之间无法互相访问ThreadLocal, 使得**资源不会被竞争**
...
另一种错误的设计思路是将ThreadLocal设计成一个共享的Map<Thread,Object>, 这样每个线程也能获取到私有的变量. 但ThreadLocal的设计初衷是为了给线程开辟线程专属的本地资源, 这种设计又会出现访问map时的资源竞争, 而且会带来Thread对象的强引用导致线程对象无法被GC, 所以设计成了每个线程拥有一个Map<ThreadLocal,Object>
...
| ----------- | ------------------------------ | ---------------------------------------- |
| 互斥        | 资源同时被多个线程使用         |                                          |
| 请求与保持  | 线程请求资源时不释放已有资源   | **一次性申请**所有资源                   |
| 不剥夺      | 已获得的资源不会被其他线程抢夺 | 申请资源失败时**主动释放**占有的资源     |
| 循环等待    | 若干线程间形成循环等待资源     | 按**固定顺序**申请资源 / 设置超时时间    |
|             |                                | 改用CAS避免加锁                          |
|             |                                | 减小锁的范围, 只对必要的读写操作加锁     |
|             |                                | 减小锁的粒度, 用多个细粒度锁代替一个大锁 |
...
当多线程访问某个类/方法/对象时, 始终能表现出正确的行为，而不需要额外的同步或协调
...
| 阻塞同步      | 非阻塞同步 | 无同步                                                       |
| ------------- | ---------- | ------------------------------------------------------------ |
| synchronized  | CAS        | 栈封闭  局部变量 / Thread Local                              |
| ReentrantLock | atomic     | 不可变对象                                                   |
|               |            | 可重入代码（Reentrant Code）,中断再执行不影响最终结果.要求不依赖公用资源、用到的状态都由参数传入、不调用非可重入的方法 |
|               |            | 基本类型读写（除long/double）具有天然原子性                  |
|               |            |                                                              |
...
重入性是为了**避免同一线程重复获取锁时的死锁**(通常为**递归**场景), 并通过**计数**的方式确保锁最终能被完全释放

1. **记录当前持有锁的线程**  申请锁的线程与当前线程相同则判定为重入
2. **持有计数** 记录锁被同一个线程反复获取的次数, **获取多少次锁就必须释放多少次**



#### 计数规则

1. 当计数器 = 0,代表锁没有被任何线程持有

2. 线程获取锁时,计数器置为1,并记录持有锁的线程
...
假设没有重入计数, 当线程递归调用同步代码块时, 释放锁会将前几次递归的加锁一起释放掉. 就导致线程的方法还未执行完毕, 这把锁又能被其他线程获取

```java
synchronized void withdraw() {   // 获取锁（计数=1）
    synchronized void log() {    // 同一线程 → 计数=2
        // ...
    }                           // 计数=1
}                               // 计数=0
```
...
JUC包下提供了很多线程安全的工具类, 调用它们的单个方法是线程安全的, 但进行复合操作时, 还是需要额外的加锁控制

另一种不加锁的办法是调用内置的原子方法, 例如`pubIfAbsent`



场景: Vector是线程安全的, 但下面的复合操作无法保证线程安全

```java
public static Object getLast(Vector list) {
	int lastlndex = list.size()
	return list.get(lastlndex);//此时容器长度可能变动
}
```
...
* **原子性**	通过互斥锁机制确保同一时刻只有一个线程能进入临界区
* **有序性**	加锁解锁有内存屏障, **禁止临界区内的指令重排到临界区之外**, 但临界区内部的方法允许重排序, 只要不影响单线程语义和`happens-before`原则
  * 加锁时：插入 `LoadLoad` + `LoadStore` 屏障, 确保临界区内的读操作 **必须从主内存加载最新值**（防止读操作重排到临界区外）
  * 解锁时：插入 `StoreStore` + `StoreLoad` 屏障, 确保临界区内的所有写操作 **必须在解锁前完成**（防止写操作重排到临界区外）

* **可见性**	当线程退出synchronized方法后, 会强制将其在**临界区内修改的共享变量刷新回主内存**, 但对于不需要进入临界区的线程(不进入synchronized方法块就直接读写共享变量), 则需要通过**volatile修饰该变量**来维护可见性
  * **加锁时**：使本地缓存失效，**强制从主内存重新加载数据**（通过 `LoadLoad` 屏障）
  * **解锁时**：将临界区内的修改**强制刷新到主内存**（通过 `StoreLoad` 屏障）
...
synchronized被编译后对应了**monitorenter + monitorexit**两个字节码指令
...
**锁的竞争本质上是看哪个线程能修改对象头的锁标志位**, 锁升级的本质是修改[对象头的锁标志位](#JOL)
...
| -------- | ------ | ---------- | ------------------------------------------------------------ |
| 无锁     | 001    |            |                                                              |
| 偏向锁   | 101    | 用户态     | 生命周期内**不存在竞争**, 不触发`mutex`指令, 一旦竞争就升级为轻量级锁 |
| 轻量级锁 | 00     | **用户态** | 撤销偏向锁, **主动地CAS自旋**尝试获取锁. 自旋超过一定次数后锁升级 |
| 重量级锁 | 10     | **内核态** | 触发`mutex`指令, 线程进入OS的**等待队列**, **被动等待**操作系统唤醒 |
| GC       | 11     |            |                                                              |

**自旋锁并没有对应的锁标志位**, 它仅仅是作为轻量级锁竞争时的一种优化策略. 况且自旋是根据运行情况动态决定的, 并不一定会进入自旋, 固定的标志位会浪费宝贵的对象头存储空间

偏向标志位只在无锁或偏向锁时存在

**重量级锁涉及到用户态-内核态的切换**, 但这并不意味着轻量级锁的性能一定优于重量级锁, 在竞争激烈的情况下, 轻量级锁会大量地自旋, 性能反而更低



### 锁自旋

当对象头的锁标志位为轻量级锁(00)时, JVM会尝试用自旋进行**自适应**优化, 需要满足以下条件才会考虑开启

1. CPU是多核的, 否则自旋没有意义
2. 锁的持有时间预计很短
3. 根据**历史自旋的成功率**决定是否自旋
...
新线程可以无视同步队列中正在等待的元素, 直接尝试抢锁(**插队**); 公平锁只能去排队

而进入**同步队列后的流程与公平锁一致**, 都是按FIFO先进先出的原则依次唤醒队首的线程



非公平锁是通过插队的形式, 避免了同步队列中的线程状态从阻塞切换为可运行, **减少了线程上下文切换的次数**, 但也可能造成部分线程在同步队列中的长时间等待(**饥饿**)
...
|                | lock()                          | tryLock()                      |
| :------------- | :------------------------------ | :----------------------------- |
| **阻塞行为**   | 会一直阻塞直到获取锁            | 立即返回，不阻塞               |
| **返回值**     | 无返回值                        | 返回boolean表示是否获取成功    |
| **可中断性**   | 支持（有`lockInterruptibly()`） | 不支持                         |
| **公平性影响** | 受公平策略影响                  | 不受公平策略影响（总是非公平） |
...
- **CPU 密集型任务(N+1)：**这种任务主要消耗CPU资源，N+1是为了防止线程偶发的==缺页中断==，或者其它原因导致的任务暂停而带来的影响,多出来的一个线程可以充分利用 CPU 的空闲时间
- **I/O 密集型任务(2N)：**系统会用大部分的时间来处理I/O，而线程在处理I/O时不占用CPU时间片，这时就可以将 CPU 交给其它线程使用

> **缺页中断**
>
> 1. 进程从辅存将数据以**页**的形式读取到主存
> 2. 在主存的空间不足时, OS会根据LRU算法(最近最少使用)决定将**部分页被换出**
> 3. 进程再次读取这份数据时, 发现页被换出, 就需要将数据载入主存
...
JMM是**抽象的内存访问模型**，而非具体的内存区域划分.  

JVM运行时数据区是对JMM的具体实现, 侧重于存储Java程序运行时所需要的数据结构和对象实例, JMM和运行时数据区是两个不同的概念



1. **控制多线程环境下对共享变量的访问(可见性、有序性和原子性),** 从而**解决CPU缓存一致性问题**, 而final常量 以及 栈中的局部变量 是线程安全的, 不在JMM的内存模型规划中
   * **共享内存**  所有线程共享**主内存(OS层面的主存)**, 但不能直接读写主内存, 必须将数据拷贝到**工作内存（寄存器、CPU 缓存等）**后再操作数据. 工作内存是线程私有的
   * **隐式通信**  上一个线程将数据从工作内存写回至主内存, 下一个线程就能读到新数据, 实现了线程之间的隐式通信
   * **显式同步**  在程序中可以显式地控制多线程访问主内存的顺序, 来让线程能够成功读取到上个线程修改过后的数据

2. **通过抽象屏蔽底层硬件差异**, 仅规范线程如何通过工作内存与主内存交互，不关心具体硬件实现, 保证各个平台都有一致的内存访问效果

> 不同 CPU 架构的内存模型不同，如x86是强内存模型, ARM是弱内存模型

![](C:\Users\hz\IdeaProjects\notes\assets\68747470733a2f2f63646e2e6a7364656c6976722e6e65742f67682f736d696c654172636869746563742f6173736574732f3230323130322f32303231303431363232313332382e706e67)



### 主内存

所有线程共享的变量存储区域，规范了多线程环境下变量的**可见性**和**访问规则**

包含: 堆中的对象字段, 静态变量, 运行池常量池的引用

主内存通常映射到**OS层面的主存**, 但这是 JVM 的实现细节，而非 JMM 的强制要求



**主内存 ≠ 堆**, 主内存包括堆中的对象, 方法区的静态变量等共享数据, 不仅仅局限于堆. 主内存也是逻辑上的概念, 而堆是物理区域
...
线程私有, 线程对变量的操作在工作内存中完成，结束后放回主内存
...
1. **原子性是基础**：没有原子性，其他特性无从谈起
2. **可见性是保证**：确保修改后的值能被其他线程看到
3. **有序性是补充**：防止指令重排序带来的问题
...
>
> 被 volatile 修饰的long/double是支持原子性的
...
为了保证**线程中的局部变量不被别的线程访问**, 栈线程私有, **线程死亡时自动释放栈空间**(栈不需要GC)

栈中的变量按定义顺序依次压栈，**相邻变量的地址之间不会存在其它变量**。栈的内存地址由高到低，**后定义的变量地址低于先定义的变量**

==栈是运行时单位，解决方法调用/执行，堆是存储单位，解决数据存储==

![](image.assets/8442519f-0b4d-48f4-8229-56f984363c69.png)



#### 虚拟机栈

由若干栈帧组成, ==大小在编译时确定==,在方法运行期间不会改变大小. 除了部分本地方法用的是本地方法栈, 其余所有的java方法调用都是通过虚拟机栈实现的

栈帧由局部变量表、操作数栈、动态链接、方法返回地址组成

* 局部变量表  存放编译期间已知的各种数据: 方法入参 / 局部变量
  * **局部变量不会被赋初值**,不像类变量在加载过程中有准备阶段
* 操作数栈  用于存放方法执行过程中产生的临时变量, 或者是上一个栈的返回值
  * Jvm的解释执行引擎基于操作数栈

* ==动态链接==,指向方法区中的方法表,从而支持方法调用的动态链接.class文件的常量池中有大量符号引用，字节码中的方法调用以常量池指向的方法的符号引用作为参数,这些符号引用一部分会在类加载阶段（解析）或首次使用的时转化为直接引用，这种转化成为静态解析，另一部分成为动态连接
* 方法返回地址  不管方法是正常还是异常返回, 栈帧都会被弹出, 代表着栈的生命周期结束

  * 正常返回：执行引擎遇到返回的字节码指令，将返回值传递给上层的方法调用者,**栈帧被弹出. 恢复调用方的局部变量表和操作数栈，并把返回值压入调用方的操作数栈，调整PC计数器,执行下一条指令**
  * 异常返回：遇到未处理的异常(本地异常表没有匹配的异常处理器),执行引擎不会读取方法返回地址，调用者不会得到返回值,**栈帧被弹出**
...
![](image.assets/1d0b41f67917d47e27d8f4693da816c3.png)
...
本地方法栈只**为JNI调用本地方法服务**, 存储的内容与虚拟机栈相近
...
![img](image.assets/a107b95c2507b6c054a3de2fd8b365a8.png)

> 《Jvm规范》定义了方法区是JVM 运行时被线程共享的一块内存，但只规定方法区的概念和作用，并没有规定如何实现,也没有规定如何GC
>
...
jdk1.8之前, **永久代PermGen**是方法区的实现. jdk1.8后, **元空间Metaspace**是方法区的实现. 永久代的数据被逐步拆分到了堆和元空间中

1. 从jdk6开始, Hotspot开发团就着手用**本地内存native memory**来代替永久代实现方法区

2. jdk7已经将字符串常量池, 静态变量从永久代移到了堆中, 但运行时常量池还在永久代

3. jdk8正式废除了永久代, 将class信息, 运行时常量池等移动到了元空间





元空间采用的是本地内存, 只受限于服务器可用的内存限制, 相较于永久代能加载更多类的元数据. 在本地内存无法满足内存分配需求时,也会OOM`java.lang.OutOfMemoryError: MetaSpace`

==方法区GC== 1. [类卸载](#卸载) 2. 常量回收(没有任何引用指向该常量)

```shell
-XX:MetaspaceSize=N #Metaspace初始大小
-XX:MaxMetaspaceSize=N #Metaspace最大大小,默认为unlimited，只受系统内存的限制
...
* class文件常量池: 存放**编译期被确定**的**字面量**和**符号引用**,这部分数据在**类加载**后进入运行常量池存放
...
- 运行时常量池
  ...
  - **运行期间新生成的常量**,如[String.intern](#intern)

- **字符串常量池**：在堆中生成的字符串都被维护在字符串常量池, **字符串常量池只存放字符对象的引用**, 指向了堆中的字符对象
...
**字节码解释器通过改变计数器值来依次读取字节码指令**，从而实现流程控制：顺序执行、选择、循环、异常处理
...
堆外内存直接**受操作系统管理**, 在io通信过程中, 堆外内存可以**避免内存在堆内和堆外间的拷贝**

堆外内存不属于JVM运行时数据区, **JMM 仅部分约束堆外内存**（如 `DirectByteBuffer` 的引用）
...
| **对比项**          | 堆                      | 堆外内存                                          |
| :------------------ | :---------------------- | :------------------------------------------------ |
| **管理方式**        | GC自动回收              | **不会被gc**, 只能手动回收`Unsafe.freeMemory()`   |
| **存储内容**        | Java 对象实例           | 二进制数据（如 `ByteBuffer`、JNI 分配的本地内存） |
| **访问速度**        | 较慢（GC 可能引发 STW） | 较快（无 GC 开销）                                |
| **是否受 JVM 限制** | 是（受 `-Xmx` 限制）    | 否（仅受系统内存限制）                            |
...
-XX:+UseCompressedClassPointers #类型指针压缩(8->4字节), 默认开启.4字节最多代表32GB的内存空间,超过32GB将无法开启指针压缩
...
| 对齐填充 padding        |       | 为了便于读取, 规定对象的大小必须是8的整数倍, 否则进行填充 |



**markword内存分布**

![markword](image.assets/image-20240419164420250.png)
...
JIT会将热点代码及时编译成机器码，下次无需编译/解释. 并有一套逻辑判断是否热点代码, 比如是否为循环
...
类在**第一次使用时**, 类加载器从class文件**加载**字节流, 并对字节流数据进行验证 准备 解析 初始化, 最终生成可以被JVM直接使用的class对象(类的结构信息：构造函数，属性和方法等),==借由class对象可以间接调用类方法->反射==

类加载不是在运行时一次性加载所有类,避免占用大量内存 -> 类加载只有一次,实例化有多次

**加载 验证 准备 解析 初始化**	5个阶段



数组类对象是JVM直接在动态内存中构造出来的, 但数组的组件类型由类加载器加载. 例如 int[] ， char[] 组件类型不是引⽤类型，由引导类加载器加载，⽽类似Integer[]这种组件 为引⽤类型的，就按照正常的类加载步骤加载



#### 加载

1. **类加载器**通过类完全限定名(com.xxx.class)将**class文件的二进制字节流**读入内存
...
| ------------ | ------------------------------------------------------------ |
| 元数据检验   | 对元数据信息进行语义分析 (是否有父类, 是否实现了抽象的方法)  |
| 字节码检验   | 通过数据流和控制流分析, 确定程序语义是合法、符合逻辑的       |
| 符号引用检验 | 验证将符号引用转化为直接引用的时候是否合法 (是否引用到了无法访问的资源) |



#### 准备

1. 为static变量分配方法区内存 ==(半初始化)==, 并赋初始值
2. 为**static final变量赋指定的值**
...
**类实例化有可能在类初始化前进行**: 在实例化时发现类未加载, 就会先加载类, 再继续实例化

```java
public class Test {
    private static final Test test1 = new Test();//1.在类加载时,出发了提前初始化
    public static int num1, num2 = 2;//3.初始化结束后,继续进行变量赋值,使得num2从1变为2

    private Test() {
        //2.初始化过程中, 对成员变量进行了自增操作
        System.out.println("类初始化-num1=" + ++num1 + ",num2=" + ++num2);
    }

    public static void main(String[] args) {
        System.out.println("类加载完毕后-num1=" + num1 + ",num2=" + num2);
    }
}

-------打印结果-----
类初始化-num1=1,num2=1
类加载完毕后-num1=1,num2=2
-------如果将num1,num2的赋值语句移动到第一行-----
类初始化-num1=1,num2=3
类加载完毕后-num1=1,num2=3
此时就是先成员变量赋值,再初始化,导致num2=3
```
...
1. 堆中不存在该类的**实例对象**, 该类的**class对象**也没有被任何地方引用到
2. 加载该类的**ClassLoader**被GC
3. 该类不会被任何**反射**方法访问
...
> 《Java虚拟机规范》
>
> 虽然定义了boolean这种数据类型，但JVM没有任何供boolean值专用的字节码指令
>
> 在编译时, **boolean变量会被转化为int**, 此时是4字节，**boolean数组会被编码成byte数组**, 此时是1字节



因为java是强类型语言,void也有对应的类Void,并且被final修饰,表示不可被继承
...
\u0000代表一个不可视字符, 所以当打印它时,输出的是一个空字符串. \u0020代表空格, 这两者并不相等

\u0000中包含一个编码为十六进制0的字符，所以长度是1. 而`''`中不包含任何字符, 所以长度为1

```java
char a = 0;	//\u0000
char b = '\u0000'; //a == b
char c = '';
//a.length=1; c.length=0;
```
...
如果要实现0.1+0.2=0.3,可以考虑用BigDecimal
...
**用双引号声明的字符串, 会在类加载阶段在当前类的字符串常量池创建对象**, 不会在堆中创建对象

非常量池中的字符串对象, 也可以调用`intern`放入常量池

```java
//字符串常量池: 增加"he","llo" 2个堆中引用
//堆: 增加"he","llo","hello" 3个字符串对象 (字符串拼接在底层上是通过StringBuilder#append实现的,最后调用toString转为字符串)
String s1 = new String("he") + new String("llo");
//字符串常量池无变化, 堆中再次新增"hello"对象
String s2 = new String("he") + new String("llo");
//字符串常量池新增s1的堆中地址,即s1和s3指向同一个堆中地址
String s3 = s1.intern();
//"hello"在字符串常量池已存在,s2.intern返回了s1的堆中地址,此时s1,3,4指向同一个堆中地址
String s4 = s2.intern();
...
* ASCII编码是仅适用于美国的字符集, 一个ASCII码的长度为8bit, 不过最高位固定为0作为校验位, 所以只定义了2^7=**128个字符**, 其中包含了33个无法显示的控制字符(回车,删除等).  **ASCII 扩展字符集**则支持256个字符
* GB2312编码中, 共收录 6700 多个汉字，基本涵盖了绝大部分**常用汉字**. 但不支持绝大部分的生僻字和繁体字

- GBK 编码是对GB2312的拓展, 收录了2w+汉字

* Unicode字符集中包含了世界上几乎所有已知的字符, 但Unicode 字符集并没有规定这些字符对应的二进制格式, 所以衍生出了UTF-8, UTF-16等编码
...
| ---------------- | ------------------------------- | --------------------------- |
| ASCII            | 1                               | 128个字符                   |
| ASCII 扩展字符集 | 1                               | 256个字符                   |
| GB2312           | 中文2字节，英文1字节            | 6700+汉字, 能够覆盖常用汉字 |
| GBK              | 中文2字节，英文1字节            | 2w+汉字                     |
| UTF-8            | 1-4字节, 汉字3字节, emoji 4字节 |                             |
| UTF-16           | 2-4字节                         |                             |
| UTF-32           | 固定4字节                       |                             |
...
* 相同点：
  * 都用 final 修饰，不能派生子类
* 不同点：
  * String内容不可变，Builder/Buffer支持在原对象上修改字符串内容, 其引用地址不变
  * ==String重写了[equals](#equals), 支持比较字符串是否相等,== Buffer、Builder未重写equals, 但可以调用`toString`后进行比较
  * Builder线程不安全
...
public class BigDecimal extends Number implements Comparable<BigDecimal> {
    private BigInteger intVal;	//整数部分
    private int scale;	//小数的位数
    private int precision = 0;	//全部的有效位数(整数位数+小数位数)
    ...
}
```
...
是Object的方法,==会释放内置对象锁,并进入同步对象的等待队列==.必须在同步方法体内使用.需要用notify()唤醒
...
* equals() 
  * 重写后, 根据方法体的逻辑进行比较, 通常是重写为比较对象的属性是否相等
  * 未重写时, 调用的是Object#equals 比较地址
* 对象相等 -> equals
...
==private方法会被隐式地指定为final==, 因为private方法本身就不会被重写
...
当子类重写父类带有thorws声明的方法时, **子类的异常范围必须小于父类**, 即子类不能抛出超出父类范围的异常

如果父类没有异常, 子类不能声明throws
...
不管有没有异常，finally都会执行,哪怕在catch中return.  因为编译后finally的代码会被复制两份到try和catch的后面

finally不会执行的场景: 线程死亡,cpu被关闭,虚拟机被终止



finnaly操作返回值对运行结果的影响: 

**在进入finnaly块之前, JVM已经将返回值压入操作数栈**, 若finnaly块重新return了结果, 则将覆盖返回值; 若finnaly块没有return, 则返回原先的结果
