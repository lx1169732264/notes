...
1. CanCommit阶段 协调者向所有参与者询问`CanCommit`请求，并等待应答；参与者反馈事务询问

2. PreCommit阶段 协调者根据上⼀阶段的反馈情况来决定是否预提交事务。有以下两种可能

   * 所有参与者都准备好了

   1. 发送预提交请求。协调者向所有节点发出PreCommit请求，并进⼊ prepared阶段
2. 参与者收到PreCommit执⾏事务操作，并将Undo和Redo⽇志写⼊本机事务⽇志.将反馈以Ack响应形式发送给协调者，同时等待最终的Commit或Abort指令
   
   * 任意⼀个参与者反馈无法提交，或者等待超时
   
   1. 协调者向所有参与者发送Rollback请求
2. 参与者中断事务
   
3. doCommit阶段 在这个阶段，会真正的进⾏事务提交，同样存在两种可能

   * 参与者和协调者之间出现**网络分区**时,参与者才一定时间内没收到协调者的`doCommit`请求,就会**自行提交事务**(因为`CanCommit阶段`得到的是正反馈.但这也可能导致了**数据不一致**的问题)

   * 协调者收到了所有参与者的Ack响应

   1. 协调者从预提交转换到提交状态，并向所有参与者，发送doCommit请求
   2. 参与者提交事务,反馈事务提交结果
   ...
   * 任意⼀个参与者反馈无法提交，或者等待超时
   ...
   2. 参与者利⽤阶段⼆中的Undo执⾏事务回滚，并反馈事务回滚结果
...
三阶段提交协议在协调者和参与者中都引⼊超时机制，并且把2PC的第⼀个阶段拆分成了两步：`CanCommit`, `PreCommit`. `CanCommit`作为一个缓冲阶段,保证了在提交事务前,各个节点都是可用状态的
...
> 网络分区
>
> 分布式系统中，多个节点之前的网络本来是连通的，但是因为某些故障（比如部分节点网络出了问题）某些节点之间不连通了，整个网络就分成了几块区域
>
> 分布式系统中节点的加入/退出都可以看作网络分区

分区容错性**P是必须满足的**,一致性C和可用性A在**出现网络分区时**,是互斥的
...
| 软状态     | Soft State            | 允许数据存在中间状态(不同节点的数据副本在同步上存在延时)，并认为中间态不影响系统整体可用性 |
| 最终一致性 | Eventually Consistent | 在分区故障恢复后，系统应达到**最终一致性**                   |

部分可用 和 最终一致 是BASE理论在AP方案上的一个延伸
