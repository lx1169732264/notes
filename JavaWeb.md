...
==三层线程分工：Acceptor、Poller和Worker==



**Acceptor**线程

* Tomcat会为web服务器开放的每个端口分配**Acceptor**线程(默认只有1个)

* 在监听到客户端连接事件后, **Acceptor**会创建`SocketChannel `套接字通道并注册到**Poller**的事件队列中



**Poller**线程

* **Poller**线程内部持有一个Java的**Selector**对象. **Selector**是非阻塞NIO模型
* 从事件队列中获取`SocketChannel `, 并注册到**Selector**对象上
* 当客户端的连接已就绪(**OP_READ可读事件**), 创建任务提交到**Worker**工作线程池



**Worker**工作线程

* 从网络数据中解析HTTP请求头/体, 封装为`HttpServletRequest`对象
* 调用`Servlet#service`执行Controller业务逻辑
* **尝试**将业务响应内容写入`SocketChannel `
  * 若一次性写入成功, 则退出`service`方法结束请求
  * 若只写入部分数据(缓冲区已满), 则将SocketChannel重新注册到Poller, 然后释放Worker线程. 后续Poller监测到**OP_WRITE可写事件**后会继续写入剩余响应数据(可能需要多次OP_WRITE循环写入直到完成)

* Worker默认会阻塞等待业务逻辑执行. 业务代码显式调用`request.startAsync()`可以开启==异步Servlet==, 这样Worker会把任务交给其他线程池处理, 自身退出`service`方法



## 响应写入协作流程

响应写入是**协作**过程, 在**缓冲区空间不足**时, Worker会无法一次性写入所有数据, 此时未写入的数据由Poller继续完成

Worker负责处理CPU 密集型业务, 不适合浪费时间等待I/O写入事件; Poller是专门预留出来处理I/O事件的



1. **网络拥塞**

- **场景**：客户端网络延迟高（如移动网络）或路由器拥堵
- **原理**：TCP 滑动窗口变小，发送缓冲区填满速度 > 数据发送速度
- **典型表现**：下载大文件时速度波动

2. **客户端处理慢**

- **场景**：客户端是低配设备（如旧手机）或浏览器卡顿
- **原理**：客户端接收缓冲区满 → TCP 通知服务器降速发送
- **数据**：Linux 默认发送缓冲区 16KB，客户端可能更小

3. **高并发大响应**

- **场景**：同时响应 1000 个客户端的大 JSON（如 2MB/个）
- **原理**：操作系统为每个连接分配的发送缓冲区有限（典型值 8-128KB）
...
sequenceDiagram
    ...
    participant SocketBuf as Socket 发送缓冲区
    participant Poller as Poller 线程 (Selector)
    
    Worker->>SocketBuf: 尝试写入响应数据
    alt 缓冲区空间充足
        SocketBuf-->>Worker: 成功写入全部数据
        Worker->>Poller: 关闭连接/结束请求
    else 缓冲区空间不足
        SocketBuf-->>Worker: 返回实际写入字节数(部分写入)
        Worker->>Poller: 注册 OP_WRITE 事件
        Worker->>Worker: 释放回线程池
        Poller->>Poller: 监听 OP_WRITE 事件
        Poller->>SocketBuf: 继续写入剩余数据
        loop 直到写完
            Poller->>SocketBuf: 分块写入
            SocketBuf-->>Poller: 返回写入状态
        end
        Poller->>Poller: 取消 OP_WRITE 监听
    end
```
...
12306一个车次会经过多个站台, 站台的库存扣减会影响到相邻两端的库存也随之减少. 

这可以理解为电商应用中的捆绑商品, A套装里捆绑了BC两个单品, 用户下单的是A, 扣减的是BC两者的库存. 在这种场景下, **行锁的竞争非常激烈**, 这意味着事务要持有很多行的锁才能完成一次下单动作, 从而线程会被迅速地占满

2. 硬座, 卧铺等等的座位类型, 车次类型

3. 中间站台卖的特别火爆, 导致两端的车票卖不出去

4. 抢票软件带来的"灰色流量"
5. 工作日和节假日的流量差距非常大, 单靠堆硬件去解决流量, 会造成巨大的资源浪费



### 方案

1. 分时段售票, 将流量均匀地拆分到多个时段
2. 候补车票, 将一部分的流量转化为异步, 支持通过消息/异步任务来慢慢地去消费它
3. 增加验证码的难度, 从而抑制爬虫软件的运行
4. 动态的云计算, 在流量高峰时期动态地去扩容集群, 从而避免资源浪费
5. 采用gemfire做缓存, gemfire是商业化的产品, 购买后会提供一整套的解决方案. redis的话是开源免费的, 但12306不差钱, 所以更倾向于花钱获取开发商的服务
