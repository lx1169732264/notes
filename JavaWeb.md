...
BASE 理论是对 CAP 中一致性和可用性权衡的结果，它的核心思想是：即使无法做到强一致性，但每个应用都可以根据自身业务特点，也就是牺牲数据的一致性来满足系统的高可用性
...
分布式系统在出现故障的时候，允许损失部分可用性来保证核心可用



> 如果系统没有发生“分区”:即网络连接通信正常，也就不存在 P 了,此时可以同时保证 CA 。因此，**如果发生“分区”，要考虑选择 CP 还是 AP**

AP 方案只是在系统发生分区的时候放弃一致性，而不是永远放弃一致性。在分区故障恢复后，系统应该达到最终一致性。这点就是 BASE 理论延伸的地方
...
系统中所有的数据副本，在经过一段时间的同步后，能达到最终一致的状态
