...
1. 同步阻塞  所有事务参与者在等待其它参与者响应的时候都处于同步阻塞等待状态，无法进行其它操作
2. 性能问题  协调者发送 prepare 请求，参与者进行事务的处理但并不提交，此时占用着资源不释放
...
在系统发生分区的时候放弃一致性，而**不是永远放弃一致性**。在分区故障恢复后，系统应达到最终一致性。这就是 BASE 理论延伸的地方



**基本可用**	分布式系统在出现不可预知故障的时候，允许损失部分可用性。在分区故障恢复后，系统应达到最终一致性。这就是 BASE 理论延伸的地方

**软状态**	允许数据存在中间状态，并认为中间态不影响系统的整体可用(允许不同节点的数据副本在同步上存在延时)
