...
降级是站在系统功能优先级的角度来应对系统故障, 只保障核心业务的正常运行



**降级的方式**

1. 延迟处理  重要业务正常处理, 次要业务先缓存起来, 靠异步任务处理
2. 在一定粒度的范围内关闭服务  例如关闭热门文章的评论区
3. 写降级  只对缓存进行操作, 然后异步任务同步到db
4. 读降级  在弱一致性的场景下, 可以只读缓存



**自动降级**

1. 超时降级  配置了超时时间和重试机制后, 通过异步探测服务的响应状况
2. 失败次数降级  一段时间内服务失败次数过多自动降级
3. 故障降级  在网络故障/服务抛出异常时, 返回默认值或缓存
4. 限流降级  在访问量过大时, 重定向到排队等待页面
...
下游服务超时无法响应上游服务时, 将不再请求下游服务, 而是返回默认值

降级是应对系统自身的故障, 熔断是应对第三方的故障
