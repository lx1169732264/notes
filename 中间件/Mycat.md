...
可以把上层看作是对下层的抽象，例如操作系统是对硬件的抽象。Mycat对数据库层做抽象，来管理这些数据库，而上层应用只需与数据库层的抽象进行交互
...
* 缺点
  * 需要增加/减少数据库服务器时,不需要改动代码
  * 支持多种类型数据库,只需在中间层配置数据库类型
...
**拦截SQL语句**，进行特定分析,如分片/路由/读写分离/缓存等，然后将此sql发往后端的真实数据库，并将结果做适当处理，最终返回给用户
