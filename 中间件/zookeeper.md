...
配置文件中并没有指定Master和Slave。但工作时通过内部的选举机制,临时产生Leader。
...
![image-20201120115625684](image.assets/image-20201120115625684.png)
...
* 单一系统映像：客户端无论连到哪一台服务器，看到的都是同样的系统视图。这意味着，如果一个客户端在同一个会话中连接到一台新的服务器，它所看到的系统状态不会更老。当一台服务器出现故障，导致客户端需要尝试连接其他的服务器时，所有滞后于故障服务器的服务器都不会接受请求，除非这些服务器赶上故障服务器。

* 持久性：更新一旦成功，就会持久存在。更新不受到服务器故障影响。
