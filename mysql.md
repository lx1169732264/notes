...
大多数数据库的隔离级别都是Read Commit,但Mysql的InnoDb引擎能够做到Read Repeate并且不造成太大的性能损失
...
**假定叶子结点存储地址,走完二级索引直接得到数据地址,不需要再走主键索引,时间成本↓**
...
**无法对物理地址进行加锁**,数据在落盘前经历redo、缓冲池等步骤，落盘前无法保证二级索引记录的物理地址是否被占用
...
避免字段我Null Mysql对可能为null的字段难以优化
