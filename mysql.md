...
以数据实体为逻辑单元.通常使用行式存储,需要为用户提供查询服务,也要修改一些记录



将整个数据实体作为一个单元进行相邻存储,容易在查询时需要**遍历全表数据**

可以通过索引来加速对指定列数据的访问,但**宽表**无法建立过多的索引
...
以数据列为逻辑单元.通常使用列式存储,批量统计用户数据,分析用户画像,产出报表



- 可以满足快速读取特定列的需求，在线分析处理往往需要在上百列的宽表中读取指定列分析
- 就近存储同一列的数据，使用压缩算法可以得到**更高的压缩率**，减少存储占用的磁盘空间
...
可以通过`UNSIGNED`属性指定为正整数, 来让字段的范围上限翻倍
...
| **列名**        | **含义**                                                     |
| --------------- | ------------------------------------------------------------ |
| id              | 标识整个查询中 SELELCT 语句的顺序。id 如果相同，从上往下依次执行。id 不同，id 值越大，执行优先级越高，如果行引用其他行的并集结果，则该值可以为 NULL |
...
Primary 复杂查询的外层	简单子查询 / 派生表(From中的子查询) / Union
...
| const           | 条件可以转化成常量 / 表中只有一条匹配行                      |
| eq_ref          | 连表时,只需要连一行记录,是比较好的join方式                   |
| ref             | 二级普通索引作为查询条件,可能有多行匹配                      |
| fulltext        |                                                              |
| ref_or_null     | 需要在ref的基础上进行二次查找,字段既需要关联条件,也可能为null |
| index_merge     | 查询条件使用了多个索引组合,此时key列会列出使用到的索引       |
...
| [Using filesort](#Usingfilesort)                             | 使用内存/磁盘排序,**无法区分出实际用的是内存还是磁盘**       |
| Using Join Buffer                                            | 连表查询时,当被驱动表的没有使用索引的时候，MySQL 会先将驱动表读出来放到 join buffer 中，再遍历被驱动表与驱动表进行查询 |
...
==Undo Log保证原子性, Redo Log保证持久性, MVCC + 锁保证隔离性==
...
| Read Uncommit读未提交 |                  |                                  |                                |
|   Read Commit读提交   |        ×         |                                  |                                |
| Read Repeate可重复读  |        ×         |          快照+==事务==           |     [MVCC/NK锁](#避免幻读)     |
|  SERIALIZABLE串行化   |        ×         |                ×                 |               ×                |
...
==隐式字段`deleted_flag` + undo log + Read View==

**修改不会直接覆盖数据，而是维护数据的多个版本,通过可见性来保证事务能看到自己应该看到的版本，使读取时不用加锁**:

通过`DATA_TRX_ID`和**快照读**维护事务的可见性,`TRX_ID`<`DATA_TRX_ID`时,事务就无法看到这行数据

通过undolog维护数据的多版本.当`TRX_ID`<`DATA_TRX_ID`时,就会遍历**undo log**找到历史版本

而删除数据也是逻辑删除,仅仅将**deleted_flag**置位,使得快照读能查到被其他事务删除的记录
...
| ------------- | ------------------------------------------------------------ | ----- |
| DATA_TRX_ID   | 最近更新行记录的事务ID (**事务ID全数据库唯一, 所以MVCC可以做到对全库数据进行快照**) | 6     |
| ~~DB_ROW_ID~~ | 没有主键&&没有唯一索引,则追加DB_ROW_ID生成聚簇索引           | 6     |
| deleted_flag  | ==标记删除,在插入数据时会复用这个空间,所以删除数据并不会导致表的大小发生变化== |       |
...
Repeatable Read	事务开启后的**首次**Read，建立Read View(之后的修改不可见,即可重复读)
...
有固定的文件大小,设置多个日志文件时，以ib_logfile[num]形式命名。多个日志文件**循环写入**，组成逻辑上无限大的空间
...
行锁/间隙锁会加在**被访问到的索引**上(不管是聚簇索引还是二级索引)
...
3. 查询使用二级索引 && 不需要回表时, 只在二级索引加锁 (此时没有访问聚簇索引)
...
==`TRX_ID`隐式字段 和 实际数据只存在于聚簇索引==, 聚簇索引加锁才能保证数据修改的一致性
...
存在表上有意向锁,但对应行的S/X锁已经释放或还未申请成功这种情况
...
----------+---------+-------------+---------+
          |TABLE    |IX           |         |
PRIMARY   |RECORD   |X,REC_NOT_GAP|15       |
```
...
DELETE并没有直接删除记录，而在隐藏字段进行删除标记，通过purge线程清理被删除的记录,所以**delete的加锁机制与select/update一致**
...
b树每层存储的数据比二叉树大得多,树的**层高比较低,读取磁盘的次数也会降低**,b+树的查询时间复杂度为**O(log n)**    3层的B+树就能够存储千万级别的数据

B树/b+树索引**所有非叶子页保存在内存/读缓存**中,一般只有叶节点需要从磁盘驱动器读取

**b+树非叶子节点不存储数据**,这导致b+树节点占用的空间更小,适合存储在内存中

b+树叶子结点是**有序的双向链表**,适合范围查找. 而b树总是需要**从根节点向下遍历子树,每次向下搜索都是随机IO**



![](image.assets/b-tree.png)

b树查找『大于4，并且小于9的数据』:4次随机IO
...
而b+树只需要1次随机IO+**n次顺序IO**,顺序IO的性能消耗远小于随机IO

**n次顺序IO**: 每个叶节点默认16kb,磁盘的页默认4kb,也就是innoDb的1页对应磁盘的4页,在范围查询访问相邻的叶节点时,需要1次顺序IO
...
hash索引的单条数据查询时间复杂度O(1),b+是O(log n)

但hash在遇到范围/顺序查询时,是需要全表扫描的
...
当操作符**左右两边的数据类型不一致**时，会发生**隐式转换**, 导致**索引失效**

1. 两个参数至少有一个是`NULL`时，不做类型转换

2. 两个参数都是 字符串 / 整数 时, 不会做类型转换

3. 十六进制的值和非数字做比较时，会被当做二进制串

4. 有一个参数是`TIMESTAMP`或`DATETIME`，另外一个参数是常量，常量会被转换为`timestamp`
...
6. **所有其他情况下，两个参数都会被转换为浮点数再进行比较**
   ...
   字符串在转化为浮点数时, 从第一个非数值的字符开始都会被替换成0  `a123bc` -> `0`  `012ab` ->`12`
...
select '123abc' = 123;	#1  被转化成浮点数进行比较了
```
...
范围查询时,结果条数过多,优化器会判定为查全表更快而不走索引
...
Innodb是按照主键索引的顺序来组织表的,**如果数据是按照索引递增顺序插入的，页写满申请新页时，是不移动原有页面的任何记录的，因此索引是紧凑的**.如果页空间满了,会触发[页分裂](#页分裂)
...
**注意类型一致避免隐式转换**	[隐式转换可能会导致索引失效](#隐式转换)



**优先选择符合存储需要的最小数据类型**

调用[IP函数](#IP函数),将IP存为整形数据
...
**避免使用子查询** 把子查询优化为join.子查询的结果集会被存储到临时表中,无法使用索引
