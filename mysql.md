...
* Read View三个全局属性
  * 列表，维护Read View生成时,系统正活跃的事务ID
  * 列表中最小的ID
  * 尚未分配的下一个事务ID(出现过的ID的最大值+1)



* Read View遵循可见性算法
  * 将要被修改的数据的当前事务ID，与其他活跃事务的ID对比（Read View维护）
    * ID < 出现过的事务ID的最大值+1,则当前事务能看到ID 所在的记录
    * ID>=列表中事务ID最小的ID ,代表ID 在Read View生成后才出现，不可见
    * ID 是否在活跃事务列表中,如果在，代表Read View生成时，事务还在活跃，没有Commit，则不可见
  * 如果ID跟Read View的属性不符合可见性，就通过DB_ROLL_PTR回滚指针去取出Undo Log中的ID再比较，即遍历链表的ID（从链首到链尾，即从最近的一次修改查起）
  * 直到找到满足特定条件的ID, 那么这个ID所在的旧记录就是当前事务能看见的最新老版本
...
- 此时还有事务1和事务3在活跃中
- 事务4在事务2快照读前一刻提交更新了
- 所以Read View记录了系统当前活跃事务1，3的ID，维护在列表trx_list上

![](image.assets/事务流程.png)

Read View不仅仅会通过一个列表trx_list来维护事务2执行快照读那刻系统正活跃的事务ID，还会有两个属性up/low_limit_id
