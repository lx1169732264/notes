...
记录表结构变更（CREATE、ALTER TABLE…）以及数据修改（INSERT、UPDATE、DELETE…）
...
* 聚簇索引
  ...
  * 定义了PK，则PK就是聚簇索引;没有PK,第一个not NULL unique列是聚簇索引;否则，InnoDB会创建一个隐藏的row-id作为聚簇索引
  * ==只有一个聚簇索引==,因为真实数据的物理顺序只有一种
  * 表的数据都是存储在聚簇索引中的
  * 叶子节点存储了整行数据,**存储结构只能为B+Tree**
* 非聚簇索引(普通索引)
  * 除了聚簇外都是非聚簇索引
* ==回表查询==:通过索引得到主键的值,再查询聚簇索引,得到数据
  * **InnoDB普通索引的叶子节点存储PK**,MyISAM的索引叶子节点存储记录指针
* 覆盖索引
...
在B-Tree索引中又分为主键索引以及普通索引 

InnoDB 和 MYISAM类型的区别在于：
...
select * from T1 where b = 12 and c = 14 and d = 3；也就是T1表中a列为4的记录
...
最左前缀匹配原则 和 联合索引的索引构建方式及存储结构有关系

idx_t1_bcd(b,c,d)，相当于创建了(b)、（b、c）（b、c、d）三个索引

联合索引是首先使用多列索引的第一列构建索引树，idx_t1_bcd(b,c,d)的例子就是先用b列构建，当b列值相等时再以c列排序，若c列的值也相等则以d列排序

这将导致b列单调递增,而cd无序,无法脱离b使用索引



所以联合索引只能从第一列开始查找。不含第一列 以及 跨列 都无法完全用到索引,如(b,d)只会用到b列索引
...
所有查询列均在索引列中(索引列覆盖了查询列)。索引中已经包含了要查询的字段的值，因此查询的时候直接返回索引中的字段，不需要回表查询



**覆盖索引必须要存储索引的列值，而hash索引、空间索引和全文索引等都不存储索引列值，MySQL只能使用B-Tree索引做覆盖索引**
...
- 索引远小于数据行的大小，因为覆盖索引只需要读取索引，减少数据访问量
...
* 建索引
  ...
  * 引导列的选择性高，需要扫描的叶子结点少，效率高(正确的顺序,排序,分组)
* 尽量把join列放到组合索引最后面 
...
* InnoDB
  ...
  * Innodb中也有**页**，在操作系统中页是一个逻辑单位，目的在于减少磁盘的交互 提升跟磁盘交互的性能，一页的默认大小为4KB
    而在InnoDB中一页的大小为16KB,在页中主要包含 **页目录** 和**用户记录**这两个区域
    InnoDB在插入的时候按照主键排序，会影响插入时候的效率，但可以提高查询的效率，当查询的数据量过多的时候，用户数据查询链表会非常的慢，这个时候就引入了**页目录**，页目录中的地址偏移量称为**槽(slot)**,新开辟的页目录可以解决长链表问题，以空间来换取时间
  * 2种存储形式
...
==避免sum的NPE==	SELECT SUM(IFNULL(g,0)) FROM table;
...
【推荐】in操作能避免则避免，若实在避免不了，评估in后边的集合元素数量，控制在1000个之内

【强制】所有的字符存储与表示，均以utf-8编码，注意字符统计函数的区别
