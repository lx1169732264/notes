...
* I 隔离性：数据库允许多个**并发**事务同时对数据进行操作，隔离性可以防止多个事务并发执行时由于交叉执行而导致数据的不一致。
...
例如 `Student`表中有字段`number`和`name`,且这两个字段组成联合索引`key(number,name)`,则使用如下查询将使用覆盖索引查询:
...
- 最好不要用随机值,随机值会使表的存储顺序随机,不能发挥数据的聚集优势
- 

![](image.assets/image-20200812100005103.png)
