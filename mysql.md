...
**默认采用AUTOCOMMIT自动提交模式**	1/ON表示启用	0/OFF表示禁用

禁用自动提交时,所有查询都在一个事务中,直到显式地执行COMMIT / ROLLBACK,事务才会结束,同时开启新事务
...
```mysql
SET TRANSACTION ISOLATION LEVEL	//设置隔离等级
```
...
页大小决定可以存储多少索引行/表行,以及一共需要多少页来存储表和索引

当表和索引被加载/重组时,每页都会预留一定比例的空闲空间,以满足加入新索引/表行
...
当表记录顺序与索引顺序一致时,操作效率高

但表记录顺序只能按主键索引的顺序来组织,通过其他索引来访问表,表中相应的记录不会按照与索引条目相同的顺序存储区,这将导致索引操作快,表操作慢
...
排队3ms **寻道4ms 半圈旋转2ms** 传输1ms = 10ms	-> 一次IO磁盘驱动器运行6ms
...
DBMS会识别出不在缓冲池的页,随后发起多页IO请求,每次请求页的数量由DBMS决定

==顺序读取能够充分发挥预读的功能==,避免重复读相邻页,提高读取速度 -> 页的大小不影响顺序读性能
...
定义了新插入行所在表页的索引,一张表只有一个

聚簇索引的目的是让表行的存储顺序尽可能地与索引行一致

如果没有聚簇索引,新插入的表行将被放置在表的最后一页或任意一个空间足够的表页上
...
==关系数据库的优势在于,用户无需关心数据的访问方式==,访问路径是DBMS由优化器确定的

在SQL被执行前,首先由优化器确定访问路径 (索引,索引访问方式,辅助式随机读)
...
索引片范围在100~110,相应的表行将被缓冲/磁盘读

**访问路径的成本取决于索引片的厚度**

厚度小->顺序扫描的索引页小->处理的索引记录小->缓冲/**磁盘读少**	根本在于减少磁盘读
...
聚簇指数据行和相邻的键值紧凑地存储在一起,==并不是一种单独的索引类型,而是数据的存储方式==

InnoDB的聚簇索引在同一结构中保存了B-Tree索引和数据行,数据行存放在索引的叶子结点		**因为不能把数据行存储在两个不同的地方,所以只有一个聚簇索引**

没有定义PK,则第一个唯一的非空列是聚簇索引;否则创建一个隐藏的主键列作为聚簇索引
...
**覆盖索引必须要存储索引的列值，而hash索引、空间索引和全文索引等都不存储索引列值，只能使用B-Tree索引做覆盖索引**

索引远小于数据行大小,只需要读索引时,将减少数据访问量,也比数据行更容易放入内存
...
* 匹配列
  * 在where子句中,有足够简单的谓词与列对应(参与索引列的定义)
  * 谓词为范围谓词,则之后的索引列都是非匹配列

* 过滤列
  * 对于最后一个匹配列之后的索引列,如果有简单的谓词与之对应,则为过滤列
  * 虽然不参与索引片的定义,但起到了减少回表/磁盘读的次数
...
统计信息包括表的记录数,表页数,叶子页数,索引的聚簇率,列的不同值个数,最大/小值

优化器进行成本估算时使用的统计信息不完整,将导致优化器做出错误决定,可能是收集统计信息时使用的选项有限,也可能是统计信息已经过时
...
Mysql不会生成查询字节码来执行查询,而是生成查询的指令树,通过存储引擎执行
...
但这不是执行顺序,Mysql从一个表开始嵌套循环,回溯完成所有表关联
...
Order By子句的引用字段都在索引中,且**与索引顺序一致**
...
SELECT CNO,FNAME
FROM	CUST
WHERE	LNAME BETWEEN LNAME AND LNAME
AND	CITY=CITY
ORDER BY FNAME
...
再把等值谓词CITY作为开头(1)
CITY,LNAME将使得索引片更窄,但CITY,FNAME没有用

添加OrderBy列(2),但仅当放在 BETWEEN 谓词LNAME前有效
索引CITY,FNAME,LNAME由于CITY只有一种(等值谓词),所以结果集根据FNAME排列,无需 ORDER BY;
索引CITY,LNAME,FNAME不是按FNAME进行排列,需要额外排序操作

因此为了满足(2),FNAME必须在LNAME前面		(2)和(1)冲突
```
...
如果程序只需要一部分的数据量,(23)会比(13)快

避免排序将使得读取数据集之后能够直接进行物化,排序也需要占用CPU资源
...
新插入的行保存在表和聚簇索引的末尾,在页被写到磁盘上前,行会被写在页上,
...
由于索引行按索引键的顺序存储,之前当索引键有一列更新时,DBMS需要将该行从旧的索引位置移动至新的索引位置(或者是原地),迁移索引需要额外的操作

现在4层索引有3层在内存中,许多索引为多列索引,使得索引键值唯一,更新不稳定列不需要迁移至新的叶子页
...
Mysql通过关键字将Sql进行解析,并生成解析树,验证关键字是否错误,关键字顺序是否正确,符号的匹配

预处理器进一步检查解析树是否合法,检查数据表和数据列是否存咋,解析名字和别名,查看是否存在歧义



**查询优化器**

将解析树转化为执行计划,语句有多种执行方式,优化器选择一条最好的方式

优化器不关心表的存储引擎,但存储引擎对优化查询有影响,优化器会请求存储引擎提供容量或某操作的开销,以及表数据的统计信息

对于select,会在解析查询之前,先检查Query Cache查询缓存,如果能找到对应的查询,就不必查询解析/优化/执行,直接返回查询缓存中的结果集
...
在Mysql服务器层有查询优化器,却没有保存数据和索引的统计信息

**统计信息由存储引擎实现**,查询优化器在生成查询计划时,需要向存储引擎获取相应的统计信息,包括每个表/索引包含多少个页面,每个表的每个索引的基数是多少,数据行和索引长度,索引的分布信息等
...
关联不只是两张表的关联,每个查询,每个片段都有可能是关联

对于Union,先将一系列单个查询结果放入临时表,然后重新读出临时表数据完成Union查询,读取临时表也是一次关联



在多表关联时,可以有多种不同的关联顺序来获得相同的执行结果

倒转关联顺序可能会让查询进行更少的嵌套循环和回溯
...
当不能利用索引排序时,Mysql需要额外的排序操作,称为文件排序fileSort

数据量小于排序缓冲区,在内存排序

排序缓冲区不足,将数据分块,对每个独立的块使用快速排序,并将各个快的结果存放在磁盘,最后进行合并,返回结果



在文件排序时所需的临时空间很大,因为在排序时需要对记录分配足够长的定长空间,例如Varchar需要分配其完整长度,如果为UTF-8字符集,还需要为每个字符预留3个字节
