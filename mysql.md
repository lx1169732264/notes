...
Mysql中各个存储引擎是完全独立的(参与者),所以跨存储引擎的事务需要一个外部的协调者

如果没有协调者,事务提交就只是顺序地要求各个存储引擎各自提交,在提交过程中发生错误,就会破坏事务特性



将二进制日志看作一个独立的引擎,在存储引擎提交事务时,需要将提交信息写入二进制日志,这就是一个分布式事务,只不过事务的参与者是Mysql本身
...
因为通信延迟和参与者本身可能失败,所以外部XA事务远比内部消耗更大

最好避免使用XA事务,这会让所有参与者都在等待

可以用别的方式实现高性能的分布式事务,例如在本地写入数据,并放入队列,然后在一个更小更快地事务中自动分发
...
服务器在需要时自动创建/释放隐式锁,并传递给存储引擎

当存储引擎感知时,会自动转换为 存储引擎特定类型的锁
...
通过Flush Tables With Read Lock 或 设置read_onnly=1 获取单个全局读锁
...
对应用程序透明,保存查询返回的完整结果,当查询命中缓存,将立即返回结果,跳过解析/优化/执行阶段

**只能降低查询执行时间,无法减少结果集传输的网络消耗**

查询缓存系统会跟踪查询中设计的每张表,**当表发生变化,和表相关的所有缓存数据都将失效**



这个机制会影响服务器性能,在很多时候需要关闭查询缓存,或者是配置很小的查询缓存空间(几十兆)

* 额外的消耗:

读查询在开始前必须先检查是否命中缓存(需要加排它锁)

当读查询可以被缓存,在执行完成后,检查查询缓存是否存在这个查询,并放入缓存

在写入后,将对应表的所有缓存设置失效

InnoDB中,多版本的特性会暂时将修改对其他事务屏蔽,在事务提交之前,表的相关查询无法被缓存(长时间运行的事务将降低缓存命中率)
...
缓存存放在hash引用表,hash中包含了查询本身/查询的数据库/客户端协议的版本



当查询语句包含不确定的数据时,不会被缓存(函数Now()/Current_Date()/用户自定义变量/临时表)
...
经常被外联的表	外联表需要不经常改动,避免缓存失效



**缓存命中率不决定查询缓存是否能够提升性能**,对于大量消耗Cpu的查询,及时命中率很低,也足以提升性能
...
Mysql总是通过创建并填充临时表的方式来执行Union查询,而无法将部分限制条件从外层下推到内层,这导致很多优化策略无法在Union中使用,需要手动将Where/Limit/OrderBy等子句下推到Union子查询
...
==除非确实需要服务器消除重复行,否则用Union All==

如果没有All,会给临时表加上Distinct,将对整个临时表进行唯一性检查
...
当表A,B根据列c关联时,如果优化器的关联顺序是B->A,就不需要在B上创建索引

确保Group/OrderBy只涉及到一张表中的列,这样才可能使用索引



**关联多张表时,优化器预估成本,调整关联的顺序**



当升级Mysql时,关联语法,运算符优先级等其他地方可能会发生变化,以前是普通关联的地方可能会变成笛卡尔积,不同类型的关联可能会生成不同的结果
