...
* Atomicity 原子性：全完成/全不完成
* Consistency 一致性：保持数据**完整性/可见性**
* I 隔离性：**允许并发事务的数据一致**
...
修改数据加X锁，直到T结束释放锁	解决ABA问题，不会丢失修改
...
**调度包含的事务都是良构的且两阶段，使得并发执行与串行执行的事务结果相同**
...
支持二级索引对于主键的覆盖索引,索引和数据保存在同一个B-Tree中
...
回表的主要消耗在于:从二级索引根据主键值再走主键索引,相当于走了2次索引,时间成本↑

而二级索引存储地址将避免从主键索引寻找行记录的时间消耗,使得回表能够直接定位到行记录的物理地址,提高回表速度
...
加锁	**无法对物理地址进行加锁**,数据在落盘前经历redo、缓冲池等步骤盘，落盘前无法保证二级索引记录的物理地址是否被占用
...
优化 == ==响应时间 + 扫描行数 + 扫描类型 + 返回行数== == 了解查询消耗的时间分布,找到合适的测量范围(只测量需要优化的部分)
...
**数字运算 代替 字符运算**	数字的比较可以在一个运算内完成,字符串需要一个字符一个字符地比较
