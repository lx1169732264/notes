...
* 二进制日志索引文件（文件名后缀为.index）,记录所有的二进制文件
* 二进制日志文件（文件名后缀为.00000*）记录数据库所有的DDL和DML(除查询)语句事件
...
* undo log 指==事务开始前==，先将需操作的数据备份,可作为数据旧版本快照,供其他并发事务快照读
  ...
  * ==update undo log==   实现**事务的原子性**,实现**MVCC**,不仅在事务回滚时需要，在快照读时也需要
    ...
    * 在快照读或事务回滚不涉及该日志时，会被**purge线程**统一清除
      ...
      * 如果某个记录的 标记删除字段 为true，并且DB_TRX_ID相对于purge线程的read view可见，那么这条记录可以被安全清除
...
事务进行快照读操作的时候生产读视图,要是用来做可见性判断

在该事务执行的快照读的那一刻，会生成数据库系统当前的一个快照，**记录并维护系统当前活跃事务ID**

用当前活跃事务ID作条件,判断当前事务能够看到哪个版本的数据
...
在事务性语句（update）执行过程中，服务器将会进行额外处理，在服务器执行时多个事务是并行执行的，为了把他们的记录在一起，需要引入事务日志的概念

在事务完成被提交的时候一同刷新到二进制日志。对于非事务性语句（insert，delete）,遵循3条规则：

1）非事务性语句被标记事务性，写入Redo

2）没标记事务性，而且Redo中没有，则直接写入二进制日志

3）没标记事务性，Redo中有，写入Redo
...
MySQL为了兼容其它非事务引擎的复制，在server层面引入了 binlog, 记录所有引擎中的修改操作，从而可以对所有的引擎进行复制,但这会导致redo log与binlog的一致性问题

MySQL通过内部XA机制解决一致性问题

* InnoDB prepare,write/sync redo log  **binlog不作任何操作**
* write/sync Binlog
  * binlog已写入，MySQL会认为事务已经提交并持久化,**发送给订阅者binlog**,==会在binlog落盘后立即将新增的binlog发送给订阅者,降低主从延迟==
  * 这时就算数据库崩溃，重启MySQL后依能正确恢复该事务

* commit (commit in memory)
  * 大部分都是内存操作(不是事务的commit)，如释放锁，释放mvcc相关的read view等
  * MySQL认为这一步不会发生任何错误，一旦发生了错误就数据库崩溃,不会回滚，MySQL无法处理
  * 在程序运行层面，只有这一步完成之后，事务导致变更才被查询出来
...
将事务的提交分两个阶段,2PC (tow phase commit)

* prepare 阶段
  * TM向所有涉及到的RM发出prepare"准备提交"请求
  * RM收到请求后执行数据修改和日志记录等处理，处理完后把事务的状态改成**"可以提交"**,然后把结果返回给TM

* commit 阶段
  * TM收到回应，如果在第一阶段有任何一个RM的操作发生了错误，或收不到回应，则认为事务失败，回撤所有RM的事务
  * 如果第一阶段中所有RM提交成功，那么TM向RM发出"确认提交"请求，RM把事务的**"可以提交"改为"提交完成"状态**，然后返回应答
  * RM收不到第二阶段的确认提交请求，也会把"可以提交"的事务回撤

- 外部与内部XA
  * MySQL中的外部XA
    * 通常意义上的分布式事务实现
  * MySQL中的内部XA
    * 单台MySQL服务器中，Server层作为TM(事务协调者，通常由binlog担当)，而服务器中的多个数据库实例作为RM，而进行的一种分布式事务，也是跨库事务,一个事务涉及到同一个MySQL服务器中的多个库。内部XA保证redo和binlog的一致性
...
==维持一个数据的多个版本，使读/写没有冲突==

提高数据库并发性能，用更好的方式去处理读/写冲突，实现不加锁/非阻塞的并发读



3个隐式字段，undo日志 ，Read View 等去完成
...
| DB_TRX_ID   | 6byte | 创建/最后一次修改的事务ID                                    |
| ----------- | ----- | ------------------------------------------------------------ |
...
* InnoDB
  ...
  * **自增长**
  * 支持事务。默认隔离级别**可重复读，通过MVCC（并发版本控制）实现**
  * **行级锁**，支持更高的并发
  * 支持外键,但增加了表的耦合度
  * 有缓冲管理，通过缓冲池加快查询的速度
  * 2种存储形式
